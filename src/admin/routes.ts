...
export function createAdminRouter(): Hono<AppEnv> {
  ...
  admin.get("/tenants/:tenantId/fiscal/registrations", async (c) => {
    const tenantId = c.req.param("tenantId");
    ...
    try {
      const client = new SupabaseAdminRpcClient(c.env);
      const registrations = await client.rpc("helvok_admin_list_fiscal_registrations", { p_tenant_id: tenantId });
  ...
  admin.get("/tenants/:tenantId/compliance/obligations", async (c) => {
    const tenantId = c.req.param("tenantId");
    ...
    try {
      const client = new SupabaseAdminRpcClient(c.env);
      const obligations = await client.rpc("helvok_admin_list_obligations", { p_tenant_id: tenantId });
